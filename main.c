...
double get_error_norm_2d(int nx, int ny, double **arr1, double **arr2)
{
  ...
  #pragma omp parallel for collapse(2) private(i, j, local_diff) reduction(+:norm_diff)
  for(i=0; i<nx; i++)
   for(j=0; j<ny; j++)
   {
...
void output_soln(int nx, int ny, int it, double tcurr, double *x, double *y, double **T,int p)
{
  int i,j;
  FILE* fp;
  char fname[100];

  sprintf(fname, "T_x_y_%06d_%0d.dat", it,p);
  //printf("\n%s\n", fname);
...
void helper(int p){
 int nx, ny;
  ...
  for(it=0; it<num_time_steps; it++)
  {
    ...
    if(it%it_print==0)
      output_soln(nx,ny,it,tcurr,x,y,T,p);
  }
